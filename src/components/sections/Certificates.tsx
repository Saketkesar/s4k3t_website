...
export function Certificates({ selectedCertId, onSelectCert }: CertificatesProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-gray-600 dark:text-slate-300 font-mono text-sm">Loading certificates...</div>
      </div>
    );
  }
  ...
  <div className="border border-gray-300 bg-white p-6 dark:card-border dark:card-bg">
          <h2 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-2">{cert.name}</h2>
          <div className="flex items-center justify-between mb-4">
            <p className="text-gray-600 dark:text-slate-300 font-mono text-xs">{cert.issuer} • {cert.year}</p>
            {cert.status && <p className="text-gray-500 dark:text-slate-300 font-mono text-xs">Status: {cert.status}</p>}
          </div>
  ...
  <h2 className="text-xl font-mono font-bold text-gray-800 dark:text-slate-100">Certificates</h2>
  <p className="text-gray-600 font-mono text-sm dark:text-slate-300">Professional achievements and learning</p>
      </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h4 className="text-gray-800 dark:text-slate-100 font-mono text-sm font-semibold">{cert.name}</h4>
                  <p className="text-gray-600 dark:text-slate-300 font-mono text-xs">{cert.issuer}</p>
                  {cert.status && (
                    <p className="text-gray-500 dark:text-slate-300 font-mono text-xs">Status: {cert.status}</p>
                  )}
                </div>
              </div>
